  ...
  return (
    <div className="space-y-8">
      <div className="text-center">
        ...
        <p className="text-gray-600">Track your progress and unlock amazing badges</p>
      </div>

      {/* Progress Overview */}
      <div className="bg-gradient-to-br from-purple-50 to-indigo-50 p-8 rounded-2xl border border-purple-200">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-purple-900">Your Journey</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-900">{earnedBadges.length}</div>
          ...
          <div className="text-center">
            ...
            <div className="text-sm text-purple-700">Rare+ Badges</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-900">
              {achievements.reduce((acc, a) => acc + a.progress, 0)}
            </div>
            <div className="text-sm text-purple-700">Total Progress</div>
          </div>
        </div>
      </div>
      ...
      {achievements.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-xl font-bold text-gray-900">Current Progress</h3>
          {achievements.map((achievement) => (
            <div key={achievement.id} className="bg-white p-6 rounded-xl border border-gray-200 hover:shadow-lg transition-shadow duration-200">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {earnedBadges.map((badge) => (
              <div
                key={badge.id}
                className="bg-white p-6 rounded-xl border-2 border-green-200 bg-gradient-to-br from-green-50 to-emerald-50 hover:shadow-lg transition-all duration-200 transform hover:scale-105"
              >
                <div className="text-center">
      ...
      {availableBadges.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-xl font-bold text-gray-900">Available Badges</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {availableBadges.slice(0, 12).map((badge) => (
              <div
                key={badge.id}
                className="bg-white p-6 rounded-xl border border-gray-200 hover:shadow-md transition-all duration-200 opacity-75 hover:opacity-100"
              >
                <div className="text-center">
                  <div className="text-4xl mb-3 grayscale">{badge.icon}</div>
                  <h4 className="font-semibold text-gray-700 mb-2">{badge.name}</h4>
      ...
      {earnedBadges.length === 0 && availableBadges.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">Complete tasks and build streaks to earn your first badges!</p>
        </div>
      )}
    </div>
  );
