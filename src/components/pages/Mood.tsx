  ...
  return (
    <div className="space-y-8">
      <div className="text-center">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">How are you feeling today?</h2>
        <p className="text-gray-600 dark:text-gray-400">Your mood helps us personalize your experience</p>
      </div>
            ...
            return (
              ...
              >
                <div className={`
                  ...
                  ${isSelected ? mood.bgColor : 'bg-gray-100 dark:bg-gray-800'}
                `}>
                  <Icon size={32} className={isSelected ? mood.color : 'text-gray-400 dark:text-gray-500'} />
                </div>
                <div className="text-center">
                  <span className={`font-semibold ${isSelected ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300'}`}>
                    {mood.label}
                  </span>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{mood.description}</p>
                </div>
                {isSelected && (
      ...
      {selectedMood && currentMood && (
        <div className={`${currentMood.bgColor} p-8 rounded-2xl border-2 ${currentMood.borderColor}`}>
          <div className="text-center mb-6">
            <div className={`w-20 h-20 ${currentMood.bgColor} rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg border border-white dark:border-gray-700`}>
              <currentMood.icon size={40} className={currentMood.color} />
            </div>
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{insights.title}</h3>
            <p className="text-gray-700 dark:text-gray-300 text-lg">{insights.message}</p>
          </div>

          {/* Mood Quote */}
          {moodQuote && (
            <div className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm p-6 rounded-xl mb-6 border border-white/50 dark:border-gray-700/50">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <h4 className="font-semibold text-gray-900 dark:text-white mb-2">Personalized Message</h4>
                  <p className="text-gray-700 dark:text-gray-300 italic leading-relaxed">"{moodQuote}"</p>
                </div>
              </div>
            </div>
          )}
          ...
          {/* Productivity Tips */}
          <div className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm p-6 rounded-xl border border-white/50 dark:border-gray-700/50">
          ...
          {/* AI Suggestion */}
          {moodSuggestion && (
            <div className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm p-6 rounded-xl mt-6 border border-white/50 dark:border-gray-700/50">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-700 dark:text-gray-300 leading-relaxed">{moodSuggestion}</p>
                </div>
              </div>
            </div>
          )}
        ...
        {selectedMood ? (
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <span className="text-gray-700 dark:text-gray-300">Current Mood</span>
              <div className="flex items-center space-x-2">
                {currentMood && <currentMood.icon size={20} className={currentMood.color} />}
                <span className="font-medium text-gray-900 dark:text-white">{currentMood?.label}</span>
              </div>
            </div>
          ...
          <div className="flex items-center justify-center py-8 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg">
            <div className="text-center">
              <Cloud className="w-12 h-12 text-gray-400 dark:text-gray-500 mx-auto mb-4" />
              <p className="text-gray-600 dark:text-gray-400 font-medium">Select your mood to see personalized insights</p>
              <p className="text-sm text-gray-500 dark:text-gray-500 mt-2">Your mood helps us tailor your experience</p>
            </div>
          </div>
        )}
      </div>
